	...
	char expected, char actual)
{
	char buf[STRING_MAX];
	if (expected == actual) return;
	sprintf(buf, "expected <%c> (ascii code %02x) but was <%c> (ascii code %02x)", expected, expected, actual, actual);
	CuFail_Line(tc, file, line, message, buf);
}
	...
	void* expected, void* actual)
{
	char buf[STRING_MAX];
	if (expected != actual) return;
	sprintf(buf, "pointer expected to differ from <0x%p>", expected);
	CuFail_Line(tc, file, line, message, buf);
}
